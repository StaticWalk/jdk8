 ...
 *
 *
 * 红黑树基于二叉树，1.节点都有颜色；2.在插入和删除是，要遵循规则：1).颜色是红或黑2).根节点总是
 * 		黑色 3).节点是红，子节点必黑 4).根到子叶或空子节点的路径，包含黑点数相同(相同的黑色高度)
 * 基本操作：添加，删除和旋转。平衡性修正：变色 > 左旋 > 右旋
 * 变色和旋转条件：1.插入节点父节点和叔叔节点（祖父节点另一个子节点）均红色   2.插入节点的父节点红色
 * 		，叔叔节点黑，插入节点是父节点的右子节点   3.插入节点的父节点红色，叔叔黑色，插入点是左子节点
 *
 *
 * 二叉树的优点是插入随机数据，执行效果很好；对于有序数据，二叉搜索树的执行速度变得很慢/插入数值有序时，
 * 		二叉树是非平衡的，排在一条线上其实就变成了一个链表，它的快速查找、插入和删除指定数据的能力丧失。
 * 为了能以O(logN)来搜索一棵树，应保证树是大致平衡的（树中每个节点的左边后代数目和在右边后代数目应该大致
 * 		相等），红黑树可以在插入一个数据项后，检查会不会破坏树的特征，破坏了会改变树的结构来纠正。
 *			@TreeMap.java  fixAfterInsertion()
 */
